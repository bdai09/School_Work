...
namespace AMA {
	...
	std::fstream& Product::store(std::fstream& file, bool newLine) const {
		...
		file
			...
			<< prod_unit << seperate << istaxable << seperate << prod_price << seperate << quantity_hand << seperate << quantity_need; //<< seperate << state_prod;
	...
	std::fstream& Product::load(std::fstream& file) {
		...
		file.clear();
		file.close();
		Product Temp=Product(sku_, name, unit, quantity, taxed_!=0, price_, qtyNeeded);
		*this = Temp;
		return file;
	}
	...
	std::ostream& Product::write(std::ostream& os, bool linear) const {
		if (state_prod.isClear()) {
			if (linear == true) { //if bool parameter true output following format
				...
				os << "Name (no space): " << prod_name << std::endl;
				os << "Price: " << price() << std::endl;
				if (istaxable)
					os << "Price after tax: " << cost() << std::endl;
				else
					os << " N/A" << std::endl;
				os << "Quantity on hand: " << quantity_hand << std::endl;
				os << "Quantity needed" << quantity_need << std::endl;
			}
		}
		return os;
	}
	...
	bool Product::operator>(const Product& srcprod) const {
		bool result = (prod_name>srcprod.prod_name);
		return result;
	}
	...
	std::istream& operator>>(std::istream& ist, Product& product) {
		product.read(ist);
		return ist;
	}
	//add total cost of Product to double received, return updated double
	double operator+=(double& src_add , const Product& product) {
